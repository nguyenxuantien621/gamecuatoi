...
// Cập nhật vị trí của các viên đạn và xử lý va chạm
void updateBullets() {
    // Duyệt ngược vector để có thể xóa phần tử khi cần
    for (int i = bullets.size() - 1; i >= 0; i--) {
            ...
            for (int c = 0; c < OBSTACLE_COLS && !triggerExplosion; c++) {
                if (obstacles[r][c].w > 0 && checkCollision(bullet.rect, obstacles[r][c])) {
                    if (bullet.large) {
                        ...
                        // Với đạn nhỏ: tiêu diệt trực tiếp chướng ngại vật
                        obstacles[r][c].w = 0;
                        obstacles[r][c].h = 0;
                    }
                    removeBullet = true;
        ...
        for (int e = 0; e < ENEMY_COUNT && !triggerExplosion; e++) {
            if (enemies[e].w > 0 && checkCollision(bullet.rect, enemies[e])) {
                if (bullet.large) {
                    triggerExplosion = true;
                } else {
                    enemies[e].w = 0;
        ...
        if (triggerExplosion && bullet.large) {
            ...
            for (int e = 0; e < ENEMY_COUNT; e++) {
                if (enemies[e].w > 0 && checkCollision(explosion, enemies[e])) {
                    enemies[e].w = 0;
                    enemies[e].h = 0;
                }
            }
            ...
            if (checkCollision(tank, explosion)) {
                // Ví dụ: đặt lại xe tăng về vị trí ban đầu
                tank.x = 0;
                tank.y = (GRID_SIZE - 1) * CELL_SIZE;
            }
        }
...
void render() {
    ...
    for (const auto& bullet : bullets) {
        // Đạn nhỏ: màu trắng; đạn lớn: màu đỏ
        if (bullet.large)
